...
class ProcessRewardDataset(Dataset):
    ...
    def __init__(
        ...
        # chat_template
        self.input_keys = getattr(self.strategy.args, "input_key", None)
        self.label_key = getattr(self.strategy.args, "label_key", None)

        # Store the processed data in class attributes
        self.inputs = [dataset[self.input_keys[0]], dataset[self.input_keys[1]], dataset[self.input_keys[2]]]
        self.labels = dataset[self.label_key]
        length = len(self.labels) // scale_off
        self.inputs = [self.inputs[0][0:length], self.inputs[1][0:length], self.inputs[2][0:length]]
        self.labels = self.labels[0:length]
        print('===========================')
    ...
    def __getitem__(self, idx):
        final_input = self.inputs[0][idx] + ' ки '.join(self.inputs[1][idx]) + self.inputs[2][idx]
